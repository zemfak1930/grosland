...
const createCheckbox = (testList1) => {
    result = [];
...
const createParamsFilter = (testList1) => {
    result = {};
...
const filterByParameters = (properties, testList1) => {
...
//  Function for downloading user and parameter data -------------------------------------------------------------------
async function loadData() {
  try {
    ...
    // Getting parameters for filtering areas --------------------------------------------------------------------------
    const paramsResponse = await fetch('/api/parameters');
    const paramsData = await paramsResponse.json();
    testParams = paramsData;
    paramsFilter = createParamsFilter(testParams);
...
loadData().then(() => {
    ...
    //  RestoreView ----------------------------------------------------------------------------------------------------
    if (!map.restoreView()) { map.setView([47.07, 29.9], minZoom) };
    ...
    for (const key in mainLayers) {
        ...
        .on('click', function(event) {
            ...
            if (properties != 0) {
                mainLayers[key].overlay.setFeatureStyle(
                    cadnum,
                    createStyle(
                        desiredFillColor=mainLayers[key].color,
                        desiredOpacity=0.4
                    )
                );
            };
